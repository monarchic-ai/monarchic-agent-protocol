...
script_label="test-project-state-artifact-report-new-file-path-existence"
...
missing_path = "scripts/__project_state_missing_new_file_path_check__.sh"
if report["new_files"]:
...
if ! grep -q "new_files reference missing file path(s)" "${PROJECT_STATE_ARTIFACT_STDERR_LOG}"; then
