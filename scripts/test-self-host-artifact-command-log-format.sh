...
if [[ ! -x "${check_script}" ]]; then
  echo "[test-self-host-artifact-command-log-format] Missing executable: ${check_script}" >&2
  exit 1
fi
...
if command -v python >/dev/null 2>&1; then
  ...
  echo "[test-self-host-artifact-command-log-format] python or python3 is required" >&2
  exit 1
fi
...
if ! bash "${tmp_repo}/scripts/test-self-host-artifacts.sh" >/dev/null 2>"${stderr_log}"; then
  echo "[test-self-host-artifact-command-log-format] Expected baseline fixtures to pass." >&2
  cat "${stderr_log}" >&2
  exit 1
fi
...
if [[ "${exit_code}" -eq 0 ]]; then
  echo "[test-self-host-artifact-command-log-format] Expected non-contiguous command index to fail." >&2
  exit 1
fi

if ! grep -q "command index must be a contiguous integer sequence" "${stderr_log}"; then
  echo "[test-self-host-artifact-command-log-format] Unexpected stderr output for command log format check:" >&2
  cat "${stderr_log}" >&2
  exit 1
fi

echo "[test-self-host-artifact-command-log-format] PASS: command log format validation is deterministic."
