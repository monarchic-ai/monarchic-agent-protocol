...
test_name="test-self-host-artifact-milestone-title-uniqueness"
...
milestones[1]["title"] = milestones[0]["title"]
...
if [[ "${exit_code}" -eq 0 ]]; then
  echo "[${test_name}] Expected duplicate milestone title to fail." >&2
  exit 1
fi

if ! grep -q "Milestone titles must be unique, found duplicates" "${stderr_log}"; then
  echo "[${test_name}] Unexpected stderr output for duplicate-title check:" >&2
  cat "${stderr_log}" >&2
  exit 1
fi

echo "[${test_name}] PASS: duplicate milestone title checks are deterministic."
