#!/usr/bin/env bash
set -euo pipefail

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
check_script="${repo_root}/scripts/test-project-state-artifacts.sh"
script_label="test-project-state-artifact-command-log-path-helper"

source "${repo_root}/scripts/project-state-command-log-test-lib.sh"

project_state_command_log_setup "${script_label}" "${repo_root}" "${check_script}"
trap 'project_state_command_log_cleanup' EXIT

project_state_command_log_reset_and_assert_baseline

command_log_relative_path="$(project_state_command_log_relative_path)"
tmp_repo_root="$(project_state_command_log_tmp_root)"
stderr_log_path="$(project_state_command_log_stderr_log_path)"
python_cmd="$(project_state_command_log_python_cmd)"
selected_python_cmd="$(project_state_command_log_choose_python "${script_label}")"
command_log_path_in_root="$(project_state_command_log_path_in_root "${tmp_repo_root}")"

if [[ -z "${command_log_relative_path}" ]]; then
  echo "[${script_label}] Expected command-log relative-path helper to return a path." >&2
  exit 1
fi

project_state_command_log_assert_core_path_listed "${command_log_relative_path}"

first_core_relative_path="$(project_state_command_log_first_core_path)"
project_state_command_log_assign_existing_first_core_path_pair_in_tmp_root paired_first_core_relative_path paired_first_core_path_in_root

if ! project_state_command_log_assert_core_path_listed "${first_core_relative_path}"; then
  echo "[${script_label}] Expected first core-path helper to resolve one wrapper-owned core path, found ${first_core_relative_path}." >&2
  exit 1
fi

first_core_path="$(project_state_command_log_first_core_path_in_tmp_root)"
first_core_path_in_root="$(project_state_command_log_first_core_path_in_root "${tmp_repo_root}")"

command_log_path="$(project_state_command_log_tmp_path)"
expected_command_log_path="$(project_state_command_log_tmp_path_for_relative_path "${command_log_relative_path}")"

if [[ "${command_log_path}" != "${expected_command_log_path}" ]]; then
  echo "[${script_label}] Expected temp command-log path ${expected_command_log_path}, found ${command_log_path}." >&2
  exit 1
fi

if [[ "${expected_command_log_path}" != "${command_log_path_in_root}" ]]; then
  echo "[${script_label}] Expected root-path helper to keep ${command_log_relative_path} aligned with the wrapper temp root ${tmp_repo_root}." >&2
  exit 1
fi

if [[ "${first_core_path}" != "${first_core_path_in_root}" ]]; then
  echo "[${script_label}] Expected root-path helper to keep ${first_core_relative_path} aligned with the wrapper temp root ${tmp_repo_root}." >&2
  exit 1
fi

if [[ "${paired_first_core_relative_path}" != "${first_core_relative_path}" ]]; then
  echo "[${script_label}] Expected first core-path pair helper to keep the first wrapper-owned relative path aligned with ${first_core_relative_path}, found ${paired_first_core_relative_path}." >&2
  exit 1
fi

if [[ "${paired_first_core_path_in_root}" != "${first_core_path_in_root}" ]]; then
  echo "[${script_label}] Expected first core-path pair helper to keep the rooted first wrapper-owned path aligned with ${first_core_path_in_root}, found ${paired_first_core_path_in_root}." >&2
  exit 1
fi

if [[ ! -f "${command_log_path}" ]]; then
  echo "[${script_label}] Expected baseline fixtures to restore ${command_log_relative_path}." >&2
  exit 1
fi

if [[ ! -f "${stderr_log_path}" ]]; then
  echo "[${script_label}] Expected wrapper stderr-log helper to resolve an initialized log file." >&2
  exit 1
fi

if ! command -v "${python_cmd}" >/dev/null 2>&1; then
  echo "[${script_label}] Expected wrapper python-command helper to resolve an available interpreter, found ${python_cmd}." >&2
  exit 1
fi

if [[ "${selected_python_cmd}" != "${python_cmd}" ]]; then
  echo "[${script_label}] Expected root-scoped python selection to stay aligned with initialized wrapper python state, found ${selected_python_cmd} vs ${python_cmd}." >&2
  exit 1
fi

missing_relative_path="NOT_A_COMMAND_LOG_CORE_PATH.json"

project_state_command_log_expect_failure_contains \
  "${missing_relative_path}" \
  "Expected temp-path helper to reject non-core path ${missing_relative_path}." \
  project_state_command_log_tmp_path_for_relative_path "${missing_relative_path}"

project_state_command_log_expect_failure_contains \
  "${missing_relative_path}" \
  "Expected core-path membership helper to fail for ${missing_relative_path}." \
  project_state_command_log_assert_core_path_listed "${missing_relative_path}"

project_state_command_log_expect_failure_contains \
  "not-a-shell-variable" \
  "Expected first core-path pair assignment helper to reject an invalid relative-path variable name." \
  project_state_command_log_assign_first_core_path_pair_in_root "not-a-shell-variable" paired_first_core_path_in_root "${tmp_repo_root}"

project_state_command_log_mutate_json "${command_log_path}" <<'PY'
command_log["status"] = "blocked"
PY

project_state_command_log_expect_reason_code "COMMAND_LOG_STATUS_MISMATCH" "Expected command-log JSON mutation helper to persist a deterministic status mutation."

project_state_command_log_reset_and_assert_baseline

project_state_command_log_mutate_json "${command_log_path}" <<'PY'
if not command_log["commands"]:
    raise SystemExit("Need at least one command entry for stderr helper coverage.")

command_log["commands"][0]["index"] = 99
PY

project_state_command_log_expect_stderr_contains \
  "command index must be a contiguous integer sequence" \
  "Expected stderr substring helper to detect deterministic command-log format drift."

echo "[${script_label}] PASS: command-log wrapper core-path membership, failure-output, temp-path membership enforcement, temp-root first-core-path resolution, existing first-core-path pair assignment, stderr-log, initialized plus root-scoped python selection, JSON-mutation, and stderr-substring helpers stay aligned with wrapper-owned fixtures."
