...
if [[ ! -x "${check_script}" ]]; then
  echo "[test-self-host-artifact-command-log-first-command] Missing executable: ${check_script}" >&2
  exit 1
fi
...
if command -v python >/dev/null 2>&1; then
  ...
  echo "[test-self-host-artifact-command-log-first-command] python or python3 is required" >&2
  exit 1
fi
...
if ! bash "${tmp_repo}/scripts/test-self-host-artifacts.sh" >/dev/null 2>"${stderr_log}"; then
  echo "[test-self-host-artifact-command-log-first-command] Expected baseline fixtures to pass." >&2
  cat "${stderr_log}" >&2
  exit 1
fi
...
if [[ "${exit_code}" -eq 0 ]]; then
  echo "[test-self-host-artifact-command-log-first-command] Expected a drifted first command to fail." >&2
  exit 1
fi

if ! grep -q "reason_code=COMMAND_LOG_FIRST_COMMAND_INVALID" "${stderr_log}"; then
  echo "[test-self-host-artifact-command-log-first-command] Unexpected stderr output for first-command reason code check:" >&2
  cat "${stderr_log}" >&2
  exit 1
fi

echo "[test-self-host-artifact-command-log-first-command] PASS: first command validation is deterministic."
