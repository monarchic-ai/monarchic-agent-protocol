...
script_label="test-self-host-artifact-milestone-count-types"
...
if ! grep -q "SELF_HOST_REPORT.json total_milestones must be an integer" "${SELF_HOST_ARTIFACT_STDERR_LOG}"; then
...
if ! grep -q "SELF_HOST_UPDATE.json milestones_done must be an integer" "${SELF_HOST_ARTIFACT_STDERR_LOG}"; then
