...
if [[ ! -x "${check_script}" ]]; then
  echo "[test-self-host-artifact-verification-evidence] Missing executable: ${check_script}" >&2
  exit 1
fi
...
if command -v python >/dev/null 2>&1; then
  ...
  echo "[test-self-host-artifact-verification-evidence] python or python3 is required" >&2
  exit 1
fi
