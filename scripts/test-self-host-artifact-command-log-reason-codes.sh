...
if [[ ! -x "${check_script}" ]]; then
  echo "[test-self-host-artifact-command-log-reason-codes] Missing executable: ${check_script}" >&2
  exit 1
fi
...
if command -v python >/dev/null 2>&1; then
  ...
  echo "[test-self-host-artifact-command-log-reason-codes] python or python3 is required" >&2
  exit 1
fi
...
if ! bash "${tmp_repo}/scripts/test-self-host-artifacts.sh" >/dev/null 2>"${stderr_log}"; then
  echo "[test-self-host-artifact-command-log-reason-codes] Expected baseline fixtures to pass." >&2
  cat "${stderr_log}" >&2
  exit 1
fi
...
if [[ "${exit_code}" -eq 0 ]]; then
  echo "[test-self-host-artifact-command-log-reason-codes] Expected status mismatch to fail." >&2
  exit 1
fi

if ! grep -q "reason_code=COMMAND_LOG_STATUS_MISMATCH" "${stderr_log}"; then
  echo "[test-self-host-artifact-command-log-reason-codes] Unexpected stderr output for status mismatch reason code check:" >&2
  cat "${stderr_log}" >&2
  exit 1
fi
...
if [[ "${exit_code}" -eq 0 ]]; then
  echo "[test-self-host-artifact-command-log-reason-codes] Expected non-contiguous command index to fail." >&2
  exit 1
fi

if ! grep -q "reason_code=COMMAND_LOG_INDEX_INVALID" "${stderr_log}"; then
  echo "[test-self-host-artifact-command-log-reason-codes] Unexpected stderr output for command index reason code check:" >&2
  cat "${stderr_log}" >&2
  exit 1
fi

echo "[test-self-host-artifact-command-log-reason-codes] PASS: command-log failures emit deterministic reason codes."
