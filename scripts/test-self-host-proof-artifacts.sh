...
if command -v python >/dev/null 2>&1; then
  ...
  echo "[test-self-host-proof-artifacts] reason_code=PYTHON_MISSING python or python3 is required" >&2
  exit 1
fi

if [[ ! -f "${proof_path}" ]]; then
  echo "[test-self-host-proof-artifacts] reason_code=PROOF_ARTIFACT_MISSING Missing required proof artifact: ${proof_path}" >&2
  exit 1
fi
...
def fail(reason_code: str, message: str) -> None:
    print(f"[test-self-host-proof-artifacts] reason_code={reason_code} {message}", file=sys.stderr)
    sys.exit(1)
...
try:
    ...
    fail("PROOF_JSON_INVALID", f"SELF_HOST_PROOF.json is not valid JSON: {exc}")
...
if not isinstance(proof_manifest, dict):
    fail("PROOF_NOT_OBJECT", "SELF_HOST_PROOF.json must be a JSON object.")
if list(proof_manifest.keys()) != required_keys:
    fail(
        "PROOF_KEY_ORDER_INVALID",
        "SELF_HOST_PROOF.json must use keys in this order: "
        f"{required_keys}, found {list(proof_manifest.keys())}.",
    )

task_id = proof_manifest["task_id"]
if not isinstance(task_id, str) or not task_id.strip():
    fail("TASK_ID_INVALID", "SELF_HOST_PROOF.json task_id must be a non-empty string.")

status = proof_manifest["status"]
if status not in {"pass", "fail", "blocked"}:
    fail(
        "PROOF_STATUS_INVALID",
        "SELF_HOST_PROOF.json status must be one of ['blocked', 'fail', 'pass'], "
        f"found {status!r}.",
    )

reason_code = proof_manifest["reason_code"]
if not isinstance(reason_code, str) or not reason_code.strip():
    fail("REASON_CODE_MISSING", "SELF_HOST_PROOF.json reason_code must be a non-empty string.")
...
if not is_strict_int(signature_count) or signature_count < 0:
    fail(
        ...
        "SELF_HOST_PROOF.json signature_count must be a non-negative integer.",
    )
...
if not is_strict_int(sampled_batches) or sampled_batches < 1:
    fail(
        ...
        "SELF_HOST_PROOF.json sampled_batches must be a positive integer.",
    )

proofs = proof_manifest["proofs"]
if not isinstance(proofs, list):
    fail("PROOFS_NOT_ARRAY", "SELF_HOST_PROOF.json proofs must be an array.")

note = proof_manifest["note"]
if not isinstance(note, str) or not note.strip():
    fail("NOTE_INVALID", "SELF_HOST_PROOF.json note must be a non-empty string.")

if signature_count != len(proofs):
    fail(
        "SIGNATURE_COUNT_MISMATCH",
        "SELF_HOST_PROOF.json signature_count must equal proofs length "
...
if status == "pass":
    if reason_code != "OK":
        fail(
            ...
            "SELF_HOST_PROOF.json reason_code must be 'OK' when status is 'pass'.",
        )
    if not proofs:
        fail(
            "PROOF_LIST_EMPTY",
    ...
    if reason_code == "OK":
        fail(
            ...
            "SELF_HOST_PROOF.json reason_code must not be 'OK' when status is not 'pass'.",
        )

print(
    "[test-self-host-proof-artifacts] PASS: proof artifact manifest is deterministic "
